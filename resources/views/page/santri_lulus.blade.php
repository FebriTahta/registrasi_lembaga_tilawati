...
<div class="modal fade bs-example-modal-diklat-kirim" id="modalhapus" tabindex="-1" role="dialog"
    ...
    <div class="modal fade bs-example-modal-diklat-kirim" id="modallulus" tabindex="-1" role="dialog"
        aria-labelledby="mySmallModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-md">
            <div class="modal-content">
                <div class="modal-content">
                    <div class="modal-header" style="background-color: rgb(146, 197, 146)">
                        <h5 class="modal-title mt-0 text-white">LULUSKAN SANTRI </h5>
                        <a href="#" type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </a>
                    </div>
                    <div class="modal-body">
                        <div class="form-group">
                            <p>Memindahkan santri berikut ini kedalam daftar santri yang sudah lulus ?</p>
                            <span>ATAS NAMA : </span><span id="nama_santri_lulus"></span>
                        </div>
                    </div>
                    <hr>
                    <div class="modal-footer">
                        <form id="formlulus">@csrf
                            <input type="hidden" name="id" id="id_lulus">
                            <button type="button" id="btnclose" class="close btn btn-secondary">close</button>
                            <input type="submit" id="btnlulus" class="btn" style="background-color: rgb(146, 197, 146); color:white" value="lulus">
                        </form>
                    </div>
                </div><!-- /.modal-content -->
            </div><!-- /.modal-content -->
...
@section('script')
    ...
    <script>
        ...
        $('#modalhapus').on('show.bs.modal', function(event) {
            ...
            $('#modallulus').on('show.bs.modal', function(event) {
                var button = $(event.relatedTarget)
                var id = button.data('id')
                var nama_santri = button.data('nama')
                var modal = $(this)
                $('#nama_santri_lulus').html(nama_santri);
                $('#id_lulus').val(id);
            })
        ...
        $('#formlulus').submit(function(e) {
            ...
            $.ajax({
                ...
                processData: false,
                beforeSend: function() {
                    $('#btnlulus').attr('disabled', 'disabled');
                    $('#btnlulus').val('Process...');
                },
                success: function(response) {
                    if (response.status == 200) {
                        $("#formlulus")[0].reset();
                        $('#btnlulus').val('Daftar');
                        $('#btnlulus').attr('disabled', false);
